...
from spec_augment import spec_augment, spec_augment_np, time_stretch, time_mask, freq_mask

def mel_spec_augment(mel_spectrogram):
    return spec_augment_np(mel_spectrogram)
...
class BirdAudioDataset(Dataset):
    def __init__(self, info, labels, tfms=None):
        ...
        self.augment = info['AUGMENT_TYPE']
    ...
    def __getitem__(self, i):
        ...
        if self.augment == "SPEC_AUGMENT":
            S_dB = spec_augment.mel_spec_augment(S_dB)
        elif self.augment == "SPEC_AUGMENT_TIME_STRETCH":
            time_stretch_ratio = 1.2
            time_stretch_min = 0.8
            time_stretch_max = 1.2
            S_dB = time_stretch(S_dB, time_stretch_ratio, time_stretch_min, time_stretch_max)
        elif self.augment == "SPEC_AUGMENT_TIME_MASK":
            time_mask_n = 2
            time_mask_t = 10
            S_dB = time_mask(S_dB, time_mask_n, time_mask_t)
        elif self.augment == "SPEC_AUGMENT_FREQ_MASK":
            freq_mask_n = 2
            freq_mask_f = 20
            S_dB = freq_mask(S_dB, freq_mask_n, freq_mask_f)
